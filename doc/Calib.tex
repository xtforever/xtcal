...
{\lbrace}
        ...
        fprintf(stderr,"Event  (x,y)=({\percent}d,{\percent}d){\backslash}n", x0, y0 );
        fprintf(stderr,"Screen (x,y)=({\percent}d,{\percent}d){\backslash}n", x, y );        
...
destroy({\dollar})
{\lbrace}
        XUngrabPointer(XtDisplay({\dollar}), CurrentTime );
{\rbrace}\endCode
...
{\lbrace}
        ...
        XClearWindow( d, XtWindow({\dollar}) );
        redraw({\dollar});
        XGrabPointer( d, XtWindow({\dollar}),
                      False,
               ButtonPressMask {\bar}
               ButtonReleaseMask {\bar}
               PointerMotionMask {\bar}
               FocusChangeMask {\bar}
               EnterWindowMask {\bar}
               LeaveWindowMask,
               GrabModeAsync,
